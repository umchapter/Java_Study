...
public abstract class GameObeject { // 추상 클래스
    protected int distance; // 한 번 이동 거리
    protected int x, y; // 현재 위치(화면 맵 상의 위치)
    public GameObeject(int startX, int startY, int distance) { // 초기 위치와 이동 거리 설정
        this.x = startX;
        this.y = startY;
    ...
    public boolean collide(GameObeject p) { // 이객체가 객체 p와 충돌했으면 true 리턴
